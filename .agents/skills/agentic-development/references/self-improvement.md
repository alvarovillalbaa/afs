...
| Repo learning system | `learning/` artifacts → promote into the correct AFS docs (`logs/`, `lessons/`, `facts/`, `fixes/`, `steers/`, `models/`, `reflections/`, `audits/`, `plans/`, `results/`, `specs/`, `knowledge/`, etc.) | "save what we learned", "mine this session", session end | Medium — agent-driven, 4-step loop |
